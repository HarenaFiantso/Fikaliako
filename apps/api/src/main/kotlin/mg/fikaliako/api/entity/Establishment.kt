...
/**
 * An eatery (project book ch. 6.1). Flyway owns the schema; this entity maps to
 * it and `ddl-auto: validate` checks the two agree. `position` is a PostGIS
 * `GEOGRAPHY(Point,4326)`; timestamps are DB-managed (default + trigger), so
...
class Establishment(
    ...
    // Read associations for the detail page; loaded lazily inside the read
    // transaction. Sets (not bags) + @OrderBy so each can be fetched independently.
